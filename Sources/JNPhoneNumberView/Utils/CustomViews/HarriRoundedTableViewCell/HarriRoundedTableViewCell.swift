...
class HarriRoundedTableViewCell: UITableViewCell {
    ...
    override func draw(_ rect: CGRect) {
        ...
        if !self.roundingCorners.isEmpty {
            UIUtil.round(view: self.customContainerView, frame: self.customContainerView.bounds, radious: self.cornerRadious, cornersToRound: roundingCorners)
        }
    }
    ...
    func setup(borderWidth: CGFloat, borderColor: UIColor, cornerRadious: CGFloat, isFirst: Bool, isLast: Bool) {
        ...
        self.customContainerView.layer.mask = nil
        ...
        UIUtil.addBorder(toView: self.customContainerView, width: borderWidth, color: borderColor)
